"""Utilities for handling LightGBM parameter and metric aliases."""

# This module replaces the internal Optuna functions used to normalize
# parameter and metric names.
...
# Mapping of canonical parameter names and their aliases.
_ALIAS_GROUP_LIST: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "alias_names": ["reg_alpha", "l1_regularization"],
    },
    {
        "param_name": "lambda_l2",
        "alias_names": ["reg_lambda", "lambda", "l2_regularization"],
    },
...
def _handling_alias_parameters(lgbm_params: dict[str, Any]) -> None:
    """Replace parameter aliases with their canonical names."""
    for alias_group in _ALIAS_GROUP_LIST:
        ...
        if len(duplicated_alias) > 1:
            msg = (
                f"{param_name} in param detected multiple identical aliases "
                f"{duplicated_alias}, but we use {param_name}="
                f"{lgbm_params[param_name]}."
            )
            warnings.warn(msg)


# Mapping of metric aliases.
_ALIAS_METRIC_LIST: list[dict[str, Any]] = [
    ...
    {"metric_name": "cross_entropy", "alias_names": ["xentropy"]},
    {"metric_name": "cross_entropy_lambda", "alias_names": ["xentlambda"]},
    {"metric_name": "kullback_leibler", "alias_names": ["kldiv"]},
    {"metric_name": "mape", "alias_names": ["mean_absolute_percentage_error"]},
    {"metric_name": "custom", "alias_names": ["none", "null", "na"]},
    {
        "metric_name": "rmse",
...
def _handling_alias_metrics(lgbm_params: dict[str, Any]) -> None:
    """Replace metric aliases with canonical metric names."""
    if "metric" not in lgbm_params:
        return

    if not isinstance(lgbm_params["metric"], (str, Iterable)):
        raise ValueError(
            "The `metric` parameter is expected to be a string or an "
            f"iterable object, but got {type(lgbm_params['metric'])}."
        )

    if isinstance(lgbm_params["metric"], str):
        lgbm_params["metric"] = (
            _ALIAS_METRIC_MAP.get(lgbm_params["metric"])
            or lgbm_params["metric"]
        )
        return
